...
/* 
 * This file has been modified for the Booming Music Project
 */
...
class SquigglyProgress : Drawable() {
	...
	var animate: Boolean = false
		set(value) {
			...
			heightAnimator = ValueAnimator.ofFloat(heightFraction, if (animate) 1f else 0f).apply {
				...
				duration = if (animate) 800L else 550L
				addUpdateListener {
	...
	private fun lerpInvSat(a: Float, b: Float, value: Float): Float {
		return ((value - a) / (b - a)).coerceIn(0.0f, 1.0f)
	}
}
